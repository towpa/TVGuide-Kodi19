...
class TVGuide(xbmcgui.WindowXML):
    ...
    def playChannel(self, channel):
        ...
        thread = threading.Timer(1, self.waitForPlayBackStopped)
        thread.daemon = True
        thread.start()
    ...
    def waitForPlayBackStopped(self):
        ...
        while self.player.isPlaying() and not xbmc.Monitor().abortRequested() and not self.isClosing:
            #time.sleep(0.5)
            xbmc.sleep(500)
    ...
    def isSourceInitializationCancelled(self):
        ...
        return xbmc.Monitor().abortRequested() or self.isClosing
    ...
    def onSourceProgressUpdate(self, percentageComplete):
        ...
        return not xbmc.Monitor().abortRequested() and not self.isClosing
        ...
        try:
            ...
            if control:
                ...
                try:
                    ...
                    pass
                control.setPosition(int(self._secondsToXposition(timeDelta.seconds)), int(y))

            #monit = xbmc.Monitor()

            if scheduleTimer and not xbmc.Monitor().abortRequested() and not self.isClosing:
                # Determine if have Timebar already and if do then cancel the thread
                if [ len(self.Timebar_List) > 0 ]:
                    for thred in self.Timebar_List:
                        thred.cancel()
                    self.Timebar_List = []

                #thread = threading.Timer(1, self.updateTimebar).start()
                thread = threading.Timer(1, self.updateTimebar)
                thread.daemon = True
                thread.start()

                self.Timebar_List.append(thread)
...
class ChannelsMenu(xbmcgui.WindowXMLDialog):
    ...
    def onClick(self, controlId):
        if controlId == self.C_CHANNELS_LIST:
            ...
            #channel = self.channelList[item.getProperty('idx')]
            ...
            #item.setIconImage(iconImage) - MAP
...
class ChooseStreamAddonDialog(xbmcgui.WindowXMLDialog):
    ...
    def onInit(self):
        ...
        for id, label, url in self.addons:
            ...
            item = xbmcgui.ListItem(label, addon.getAddonInfo('name'))
