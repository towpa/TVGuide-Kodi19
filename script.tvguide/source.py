...
class Database(object):
    ...
    def _invokeAndBlockForResult(self, method, *args):
        ...
        #monit = xbmc.Monitor()
        while not method.__name__ in self.eventResults and not xbmc.Monitor().abortRequested():
            #time.sleep(0.1) - changed to 500 milliseconds - MAP
            xbmc.sleep(500)
    ...
    def _getProgramList(self, channels, startTime):
        ...
        c = self.conn.cursor()
        c.execute('SELECT p.*, (SELECT 1 FROM notifications n WHERE n.channel=p.channel AND n.program_title=p.title AND n.source=p.source) AS notification_scheduled FROM programs p WHERE p.channel IN (\'' + ('\',\''.join(list(channelMap.keys()))) + '\') AND p.source=? AND p.end_date > ? AND p.start_date < ?', [self.source.KEY, startTime, endTime])
        for row in c:
    ...
    def adapt_datetime(ts):
        ...
        # http://docs.python.org/3/library/sqlite3.html#registering-an-adapter-callable
...
class XMLTVSource(Source):
    ...
    def getDataFromExternal(self, date, progress_callback=None):
        if self.xmltvType == XMLTVSource.TYPE_LOCAL_FILE:
            ...
            u = urllib.request.urlopen(self.xmltvUrl, timeout=30)
            xml = u.read()
            u.close()
